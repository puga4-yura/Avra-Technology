...
gulp.task('js-min', function () {
    gulp.src(['app/js/libs.js', 'app/js/main.js'])
        .pipe(jsmin())
        .pipe(rename({suffix: '.min'}))
        .pipe(gulp.dest('app/js'));
});
gulp.task('concat-css', function () {
  return gulp.src(['app/libs/bootstrap.css', 'app/libs/flexslider.css' ])
...
gulp.task('script', function(){
	return gulp.src(
		['app/libs/jquery.min.js', 'app/libs/jquery.flexslider.js' ])
	    .pipe(concat('libs.js'))
	    .pipe(uglify())
	    .pipe(gulp.dest('app/js/'))
});
...
gulp.task('build', ['clean', 'img', 'css-libs',  'less', 'script' ], function(){
	...
	var buldJs = gulp.src(['app/js/main.min.js', 'app/js/libs.min.js'])
